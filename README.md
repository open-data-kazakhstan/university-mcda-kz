...
- `data/data.csv`: Нүктелі үтірді (`;`) бөлгішті пайдалана отырып, Қазақстан университеттері үшін әртүрлі көрсеткіштері бар бастапқы деректер.
- `data/data_normalized.csv`: Университеттер туралы деректерді түзетілген, нормаланған және тазартылған деректер.
